...
import { getItemTotal, getTotalPrice, findBreadById } from '@/utils/calculations'
...
const defaultItem = (firstBreadId: string): { breadId: string; quantity: number } => ({
...
export function OrderForm({ breadTypes, acceptingOrders }: OrderFormProps) {
  ...
  const totalPrice = useMemo(
    () => getTotalPrice(itemDetails.map((i) => ({ unitPrice: i.unitPrice, quantity: i.quantity }))),
    [itemDetails]
  )
  ...
  return (
    <>
      ...
      >
        <CustomerFields register={register} errors={form.formState.errors} />
